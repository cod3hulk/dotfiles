...
- **aerospace** (`aerospace/aerospace.toml`) — tiling WM and hotkey daemon in one, no SIP disable required. Replaces yabai + skhd. Uses AeroSpace's built-in behaviour for floating windows (`layout floating tiling`, native `fullscreen`/`move`); no custom placement helper.
