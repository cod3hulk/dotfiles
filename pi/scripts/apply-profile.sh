...
# Keep existing auto-discovered local extensions/themes available.
...
if [[ -f "$repo_dir/pi/config/rtk-optimizer.json" ]]; then
  mkdir -p "$pi_dir/extensions/pi-rtk-optimizer"
  ln -sfn "$repo_dir/pi/config/rtk-optimizer.json" "$pi_dir/extensions/pi-rtk-optimizer/config.json"
fi
